...
export default function AppErrorBoundary() {
  ...
  const friendlyTitle = errorMeta.status >= 500 ? '系统暂时不可用' : '无法完成您的请求';
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-col items-center gap-6 rounded-3xl border border-border/60 bg-background/95 p-8 text-center shadow-lg shadow-black/5 backdrop-blur">
        ...
        <div className="grid w-full gap-3 text-left text-sm text-muted-foreground md:grid-cols-3">
          <div className="rounded-2xl border bg-muted/40 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground/70">状态码</p>
            <p className="mt-1 text-2xl font-semibold text-foreground">{errorMeta.status}</p>
            <p className="text-xs text-muted-foreground">{errorMeta.statusText}</p>
          </div>
          <div className="rounded-2xl border bg-muted/40 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground/70">访问路径</p>
            <p className="mt-1 line-clamp-2 font-medium text-foreground">{pathLabel || '/'}</p>
          </div>
          <div className="rounded-2xl border bg-muted/40 p-4">
            <p className="text-xs uppercase tracking-wide text-muted-foreground/70">时间</p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <Button variant="outline" onClick={handleBackHome} className="min-w-32 gap-2">

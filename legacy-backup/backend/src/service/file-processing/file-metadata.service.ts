...
import { TEXT_FILE_CONFIG } from './file-reader.service.js';
...
export class FileMetadataService {
  ...
  getFileBasicInfo(filePath: string): {
    ...
    return {
      ...
      ext,
      size: stats.size,
      isText: this.fileReader.isTextFile(filePath),
  ...
  async extractExifMetadata(filePath: string): Promise<Tags> {
    try {
      ...
      fileInfoLogger.debug({ file: path.basename(filePath), err }, 'EXIF元数据提取失败');
      throw err;
    }
  }
   ...
   */
  getMetadataTagsString(tags: Tags, filePath: string): string {
  ...
  async getFileDescription(filePath: string): Promise<string> {
    const fileName = path.basename(filePath);
    const isText = this.fileReader.isTextFile(filePath);
    ...
    if (isText) {
      try {
        ...
        fileInfoLogger.debug({ file: fileName, err }, '文本读取失败，尝试使用 exiftool');
      }
    }
    ...
    try {
      ...
      fileInfoLogger.debug({ file: fileName, err }, '元数据提取失败，使用备用方法');
      return this.formatFilePrompt('', '');
    }
  }

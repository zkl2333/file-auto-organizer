...
export class FileMoveService {
   ...
   */
  private async unlinkWithRetryRecursive(
    src: string,
    finalPath: string,
    attempt: number = 1
  ): Promise<void> {
    try {
      ...
      if (
        (err?.code === 'EBUSY' || err?.code === 'EACCES' || err?.code === 'EPERM') &&
        attempt < FILE_MAX_RETRIES
      ) {
   ...
   */
  private async attemptMoveRecursive(
    file: string,
    targetDir: string,
    desiredTargetPath: string,
    attempt: number = 1
  ...
  async moveFile(file: string, targetDir: string, dryRun: boolean = false): Promise<void> {
    ...
    const normalizedTargetDir =
      targetDirBase === fileBaseName ? path.dirname(targetDir) : targetDir;

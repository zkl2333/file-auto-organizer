...
export const LogRenderer: React.FC<LogRendererProps> = ({
  ...
  try {
    ...
    // 提取关键信息用于特殊显示
    const {
      file, from, to, method, reasoning, similarity, score, similar,
      // AI日志特有字段
      totalFiles, classified, tokens, promptTokens, completionTokens,
      elapsedMs, avgMsPerFile, batchSize, totalBatches, batchIndex,
      uniqueDirs, newDirs, existingDirs, sample, successRate,
      filesSample, hasMore, model, progress, taskId,
      successfulMoves, failedMoves, targetDirs, dryRun,
      errorType, filesCount,
      ...otherData
    } = rest;

    return (
      <div className={`rounded-lg border ${levelInfo.bgColor} ${levelInfo.color.includes('border') ? '' : 'border-border/20'} p-3 hover:shadow-md transition-all duration-200`}>
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 text-sm">
              <span className="text-gray-500 dark:text-gray-400 font-mono text-xs">{formattedTime}</span>
              <Badge variant="outline" className={`${levelInfo.color} ${levelInfo.bgColor} border-current/30`}>{levelInfo.label}</Badge>
              {module && <Badge variant="secondary" className="text-xs">{module}</Badge>}
              {method && <Badge variant="default" className="text-xs bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300">{method}</Badge>}
            </div>
            ...
            {/* AI日志的特殊展示 */}
            {module === 'ai' && (tokens !== undefined || totalFiles !== undefined || batchIndex !== undefined) && (
              <div className="mt-3 space-y-2">
                ...
                {(uniqueDirs !== undefined || targetDirs !== undefined) && (
                  <div className="flex flex-wrap gap-2 items-center text-xs">
                    ...
                    {newDirs !== undefined && newDirs > 0 && (
                      <Badge variant="outline" className="bg-teal-50 dark:bg-teal-950 text-teal-700 dark:text-teal-300 border-teal-200 dark:border-teal-800">
                        ✨ 新建: {newDirs}
                      </Badge>
                    )}
                    {existingDirs !== undefined && (
                      <Badge variant="outline" className="bg-cyan-50 dark:bg-cyan-950 text-cyan-700 dark:text-cyan-300 border-cyan-200 dark:border-cyan-800">
                        ♻️ 复用: {existingDirs}
                      </Badge>
                    )}
                ...
                {(elapsedMs !== undefined || successRate !== undefined) && (
                  <div className="flex flex-wrap gap-2 items-center text-xs">
                    ...
                    {successRate !== undefined && (
                      <Badge variant="outline" className={successRate >= 90 ? "bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800" : successRate >= 70 ? "bg-yellow-50 dark:bg-yellow-950 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800" : "bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800"}>
                        ✅ 成功率: {successRate}%
                      </Badge>
                    )}
                ...
                {filesSample && Array.isArray(filesSample) && filesSample.length > 0 && (
                  <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-950/50 rounded border border-blue-100 dark:border-blue-900">
                    <div className="text-xs text-blue-700 dark:text-blue-300 font-semibold mb-1">📄 文件样本:</div>
                    <div className="flex flex-wrap gap-1">
                      {filesSample.map((fileName: unknown, idx: number) => (
                        <span key={idx} className="text-xs text-blue-600 dark:text-blue-400 bg-white dark:bg-blue-900/30 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-800">
                          {String(fileName)}
                        </span>
                      ))}
                      {hasMore && <span className="text-xs text-blue-500 dark:text-blue-400">...</span>}
                    </div>
                  </div>
                )}
                ...
                {sample && Array.isArray(sample) && sample.length > 0 && (
                  <div className="mt-2 p-2 bg-purple-50 dark:bg-purple-950/50 rounded border border-purple-100 dark:border-purple-900">
                    <div className="text-xs text-purple-700 dark:text-purple-300 font-semibold mb-1">🎯 分类样本:</div>
                    <div className="space-y-1">
                      {sample.map((item: any, idx: number) => (
                        <div key={idx} className="text-xs bg-white dark:bg-purple-900/30 p-2 rounded border border-purple-200 dark:border-purple-800">
                          <div className="flex items-center gap-2">
                            <span className="text-purple-600 dark:text-purple-400 font-mono">{item.file || item.fileName}</span>
                            <span className="text-gray-400">→</span>
                            <span className="text-green-600 dark:text-green-400 font-semibold">{item.dir || item.path}</span>
                          </div>
                          {item.reason && (
                            <div className="mt-1 text-gray-600 dark:text-gray-400 italic">{item.reason}</div>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              <div className="mt-2 space-y-1 text-xs">
                ...
                {from && (
                  <div className="flex gap-2">
                    ...
                    <span className="text-gray-700 dark:text-gray-300 font-mono truncate" title={from as string}>{from as string}</span>
                  </div>
                )}
                {to && (
                  <div className="flex gap-2">
                    <span className="text-gray-500 dark:text-gray-400 font-semibold">目标:</span>
                    <span className="text-green-600 dark:text-green-400 font-mono truncate" title={to as string}>{to as string}</span>
                  </div>
                )}
                {(similarity !== undefined || score !== undefined) && (
                  <div className="flex gap-2">
                    ...
                    {similar && <span className="text-gray-500 dark:text-gray-400">← {similar as string}</span>}
                  </div>
                )}
                {reasoning && (
                  <div className="flex gap-2">
                    <span className="text-gray-500 dark:text-gray-400 font-semibold">理由:</span>
                    <span className="text-gray-700 dark:text-gray-300 italic">{reasoning as string}</span>
                  </div>
                )}
              </div>
            )}

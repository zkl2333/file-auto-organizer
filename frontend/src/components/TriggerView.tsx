...
export const TriggerView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        setCronEnabled(status.cronEnabled);
      } catch (err) {
  ...
  const handleCronToggle = async (enabled: boolean) => {
    ...
    try {
      ...
      setCronMessage({ type: 'error', text: '切换失败: ' + (err instanceof Error ? err.message : String(err)) });
      setTimeout(() => setCronMessage(null), 3000);
    } finally {
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {cronMessage && (
            <Alert className={cronMessage.type === 'success' ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}>
              <AlertDescription className={cronMessage.type === 'success' ? 'text-green-800' : 'text-red-800'}>
                {cronMessage.text}
              </AlertDescription>
            </Alert>
          )}

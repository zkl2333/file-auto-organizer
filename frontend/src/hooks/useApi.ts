 ...
 * 获取使用统计（自动刷新）
 */
export function useUsageStats(range: 'today' | 'week' | 'month' | 'all' = 'all', config?: SWRConfiguration) {
  return useSWR<UsageStats>(
    `/api/usage-stats?range=${range}`,
...
export function useTaskHistory(config?: SWRConfiguration) {
  return useSWR<{ tasks: TaskRecord[] }>(
    '/api/task-history',
    api.getTaskHistory,
 ...
 * 获取任务日志
 */
export function useTaskLogs(taskId: string | null, type: string = 'main', limit: number = 200, config?: SWRConfiguration) {

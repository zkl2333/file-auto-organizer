...
describe('FileInfoService 集成测试', () => {
  ...
  describe('向后兼容性测试', () => {
    ...
    test('应该正确处理流式读取接口', () => {
      ...
      try {
        ...
        const result = fileInfoService.readFileStream(testFile, (chunk, isLast) => {
  ...
  describe('文件描述生成集成测试', () => {
    test('应该正确生成文本文件描述', async () => {
      ...
      const content = 'This is a meaningful content for integration testing. It should be properly extracted and formatted.';
  ...
  afterAll(() => {
    // 清理可能遗留的测试文件
    const testFiles = [
      'compatibility-test.txt', 'stream-compatibility.txt', 'delegate-text.txt',
      'delegate-binary.bin', 'delegate-validate.txt', 'delegate-metadata.txt',
      'integration-text.txt', 'integration-json.json', 'integration-nonexistent.txt',
      'integration-empty.txt', 'error-binary.txt', 'error-nonexistent.txt',
      'error-size.txt'
    ];

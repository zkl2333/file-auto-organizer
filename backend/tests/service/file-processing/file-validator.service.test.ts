...
describe('FileValidatorService', () => {
  ...
  afterAll(() => {
    // 清理可能遗留的测试文件和目录
    const testFiles = [
      'validate-test.txt', 'empty.txt', 'large.txt', 'size-test.txt',
      'size-nonexistent.txt', 'regular-file.txt', 'readable.txt',
      'permission-test.txt', 'not-a-dir.txt', 'process-test.txt',
      'cannot-process.txt', 'custom-size-test.txt'
    ];

    const testDirs = [
      'test-directory', 'test-dir', 'writable-dir', 'nonexistent-dir'
    ];

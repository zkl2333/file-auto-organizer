...
describe('FileMetadataService', () => {
  ...
  describe('文本文件描述生成', () => {
    test('应该正确生成文本文件描述', () => {
      ...
      const content = 'This is a meaningful text file\nwith multiple lines\ncontaining important information';
    ...
    test('应该正确截断过长的内容', () => {
      ...
      const longContent = 'This is a very long content that should be truncated when the file description is generated to ensure it does not exceed the maximum length limit. '.repeat(10);
  ...
  describe('提示格式化', () => {
    test('应该正确格式化文件提示', () => {
      ...
      expect(formatted).toBe('元数据: Title: Test Title, Author: Test Author\n摘要: This is a test file with meaningful content');
    });
  ...
  describe('主要接口', () => {
    test('应该正确获取文件描述（文本文件）', async () => {
      ...
      const content = 'This is a meaningful text file content that should be extracted and used for file description generation.';
  ...
  afterAll(() => {
    // 清理可能遗留的测试文件
    const testFiles = [
      'basic-info.txt', 'text-description.txt', 'json-description.json',
      'json-no-keywords.json', 'empty-description.txt', 'long-content.txt',
      'exif-test.jpg', 'non-existent.jpg', 'description-main.txt',
      'binary-description.jpg', 'specific-metadata.txt', 'json-specific-metadata.json'
    ];

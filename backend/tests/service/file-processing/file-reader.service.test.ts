...
describe('FileReaderService', () => {
  ...
  describe('文本文件检测', () => {
    test('应该正确识别文本文件扩展名', () => {
      const textExtensions = [
        '.txt', '.log', '.md', '.json', '.xml', '.html', '.htm', '.css',
        '.js', '.ts', '.py', '.java', '.c', '.cpp', '.h', '.hpp', '.cs',
        '.php', '.rb', '.go', '.rs', '.kt', '.swift', '.sql', '.sh',
        '.bat', '.cmd', '.ps1', '.yaml', '.yml', '.ini', '.conf',
        '.config', '.properties', '.env', '.gitignore', '.dockerfile',
  ...
  describe('文件内容读取', () => {
    ...
    test('应该正确处理字符限制', () => {
      ...
      const longLine = 'This is a very long line that should be truncated when we apply character limits to test the functionality';
  ...
  describe('流式读取', () => {
    test('应该正确处理流式读取', () => {
      ...
      try {
        ...
        let collectedChunks = '';
        const result = fileReader.readFileStream(testFile, (chunk, isLast) => {
    ...
    test('应该支持提前停止流式读取', () => {
      ...
      const content = 'This is a longer content that we will stop reading early to test the stop functionality.';
  ...
  afterAll(() => {
    // 清理可能遗留的测试文件
    const testFiles = [
      'binary-png.bin', 'binary-jpeg.bin', 'binary-gif.bin', 'binary-pdf.bin',
      'binary-zip.bin', 'binary-rar.bin', 'binary-elf.bin', 'binary-exe.bin',
      'pure-text.txt', 'lines-test.txt', 'chars-test.txt', 'full-content.txt',
      'size-limit.txt', 'segment-test.txt', 'stream-test.txt', 'stream-stop-test.txt',
      'not-text.bin', 'public-interface-text.txt', 'public-interface-bin.bin',
      'preview-test.txt'
    ];

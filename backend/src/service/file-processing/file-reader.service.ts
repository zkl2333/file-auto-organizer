...
class FileOperationMetrics {
  private static operations = new Map<string, { count: number; totalTime: number; errors: number }>();
  ...
  static recordOperation(operation: string, duration: number, success: boolean) {
    ...
    // 记录慢操作
    if (duration > 1000) {
      fileInfoLogger.warn({ operation, duration, success }, "检测到慢文件操作");
    }
  }
  ...
  constructor() {
    ...
    setInterval(() => {
      ...
      if (Object.keys(metrics).length > 0) {
        fileInfoLogger.debug({ metrics }, "文件操作性能指标");
      }
    }, 60000); // 每分钟记录一次
  }
    ...
    try {
      ...
      for (const signature of BINARY_SIGNATURES) {
        if (sample.length >= signature.length) {
          ...
          if (matches) {
            FileOperationMetrics.recordOperation('isTextFile_binary_signature', Date.now() - startTime, true);
            return false;
          }
        }
      }
      ...
      FileOperationMetrics.recordOperation('isTextFile_content_analysis', Date.now() - startTime, true);
      return isText;
    } catch (err) {
      FileOperationMetrics.recordOperation('isTextFile_error', Date.now() - startTime, false);
      fileInfoLogger.error({ file: fileName, err }, "文件类型检测失败");
      return false;
    }
  }
    ...
    try {
      ...
      if (fd !== null) {
        try {
          ...
          fileInfoLogger.warn({ file: path.basename(filePath) }, "文件句柄关闭失败");
        }
      }
    }
  }

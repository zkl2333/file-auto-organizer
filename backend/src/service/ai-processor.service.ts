...
import { FileStatusService, FileProcessStatus, FileProcessStage, ProcessedFile, FileStatusUpdate } from "./file-status.service.js";
...
export class AIProcessorService {
  ...
  constructor(
    ...
    this.fileMoveProcessor = new FileMoveProcessorService(this.fileMoveService, this.fileStatusService);
  }
  ...
  async processAIClassification(
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        const { classifications: batchResults, tokensUsed } = await this.aiClassificationService.classifyBatch(
    ...
    const avgMsPerFile = allClassificationResults.length > 0 ? Math.round(totalElapsedMs / allClassificationResults.length) : 0;
    ...
    mainLogger.info(
      {
        ...
        totalElapsedMs,
        avgMsPerFile,
        successRate: Math.round((allClassificationResults.length / needAIClassification.length) * 100)
      },
    ...
    mainLogger.info(
      {
        ...
        elapsedMs,
        avgMsPerFile: result.successfulMoves > 0 ? Math.round(elapsedMs / result.successfulMoves) : 0,
  ...
  async processAIClassificationAndMove(
    ...
    try {
      const classificationResults = await this.processAIClassification(taskId, processedFiles, needAIClassification, knownDirs);
      ...
      const taskStatus: 'success' | 'partial' | 'failed' =
        successfulMoves === needAIClassification.length ? 'success' :
        successfulMoves > 0 ? 'partial' : 'failed';

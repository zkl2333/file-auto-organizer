...
export class MainService {
  ...
  private async performSimilarityMatching(
    ...
    for (const file of processedFiles) {
      ...
      const { bestDir, bestRelPath, bestScore } = SimilarityUtils.findMostSimilarFile(file.name, knownFiles);
      ...
      if (bestDir && bestScore >= SIMILARITY_THRESHOLD) {
        ...
        mainLogger.debug(
          { file: file.name, similar: bestRelPath ? path.basename(bestRelPath) : null, score: bestScore.toFixed(2) },
          "找到相似文件"
        );
    ...
    // 批量保存相似度匹配结果
    await this.fileStatusService.batchUpdateAndSaveFileStatus(taskId, processedFiles, similarityUpdates);
  ...
  async runOnce(dryRun: boolean = false): Promise<{
    ...
    try {
      ...
      const successfulMoves = await this.moveSimilarityMatchedFiles(
        taskId,
        processedFiles,
        similarityResults.filter(r => r.bestDir !== null).map(r => ({
          fileName: r.fileName,
          filePath: r.filePath,
          bestDir: r.bestDir!,
      ...
      const aiResult: AIProcessResult = await this.aiProcessorService.processAIClassificationAndMove(
        taskId,
        processedFiles,

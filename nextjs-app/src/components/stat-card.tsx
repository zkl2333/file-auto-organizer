...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`absolute top-0 right-0 w-24 h-24 ${config.blur} rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700`}
      />
      <CardContent className="p-6 relative">
        <div className="flex items-start justify-between mb-4">
          <div
            className={`p-3 rounded-xl ${config.iconBg} group-hover:scale-110 transition-transform duration-300`}
          >
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            <h3 className={`text-3xl font-bold ${config.valueColor}`}>{displayValue}</h3>
            <span className={`text-xs ${config.unitColor}`}>{unit}</span>
          </div>

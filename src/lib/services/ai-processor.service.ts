...
import { FileMoveProcessorService } from './file-move-processor.service';
...
export class AIProcessorService {
  ...
  constructor(
    ...
    this.fileMoveProcessor = new FileMoveProcessorService(this.fileMoveService);
  }
  ...
  async processAIClassification(
    ...
    let allClassificationResults: Array<{ fileName: string; path: string; reasoning?: string }> =
      [];
  ...
  async processAIClassificationAndMove(
    ...
    if (needAIClassification.length === 0) {
      logger.info({ taskId }, 'AI分类流程跳过: 无需分类的文件');
